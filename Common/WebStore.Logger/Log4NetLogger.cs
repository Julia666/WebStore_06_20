...
namespace WebStore.Logger
{
    public class Log4NetLogger : ILogger
    {
        ...
        public IDisposable BeginScope<TState>(TState State) => null; // логгер не будет поддерживать областей журналов

        public bool IsEnabled(LogLevel Level) // разрешено ли ведение журнала для заданного уровня
        {
            switch (Level)
            {
            ...
            Func<TState, Exception, string> Formatter)
        {
            ...
            switch (Level)
            {
                ...
                case LogLevel.Trace: _Log.Debug(log_message); break;
                case LogLevel.Debug: _Log.Debug(log_message); break;
                case LogLevel.Information: _Log.Info(log_message); break;
                case LogLevel.Warning:_Log.Warn(log_message); break;
                case LogLevel.Error:_Log.Error(log_message,Error); break;
                case LogLevel.Critical: _Log.Fatal(log_message,Error); break;
            }
        }
    }
}